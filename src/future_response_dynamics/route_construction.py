...
def future_map(model: DeterministicBeliefModel, z: torch.Tensor, future_actions: torch.Tensor, output_weights: torch.Tensor) -> torch.Tensor:
    if future_actions.shape[0] != HORIZON:
        raise RuntimeError(f"FUTURE_HORIZON_SHAPE_FAILURE:{tuple(future_actions.shape)}")
    if output_weights.ndim != 1 or output_weights.shape[0] != 8:
        raise RuntimeError(f"OUTPUT_WEIGHT_PRIMITIVE_AXIS_FAILURE:{tuple(output_weights.shape)}")
    outputs = []
    state = z
    for step in range(HORIZON):
        state = model.transition(state, future_actions[step])
        decoded = model.decode(state)
        if decoded.shape[-1] != 8:
            raise RuntimeError(f"DECODED_PRIMITIVE_AXIS_FAILURE:{tuple(decoded.shape)}")
        outputs.append(decoded * output_weights)
    stacked = torch.stack(outputs, dim=0)
    if stacked.shape[-2:] != (HORIZON, 8) or stacked.numel() != 64:
        raise RuntimeError(f"STACKED_FUTURE_SHAPE_FAILURE:{tuple(stacked.shape)}")
    return stacked
